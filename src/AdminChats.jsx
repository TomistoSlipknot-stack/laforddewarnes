...
export default function AdminChats({ network }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: 'var(--fw-bg, #fafafa)' }}>
      {/* Sidebar - chat list */}
      <div style={{ width: activeRoom ? 'min(220px,35%)' : '100%', borderRight: '1px solid var(--fw-cardBorder, #e0e0e0)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '12px', borderBottom: '1px solid var(--fw-cardBorder, #e0e0e0)' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[
              ...
              <button key={f.id} onClick={() => setFilter(f.id)}
                style={{ flex: 1, background: filter === f.id ? '#003478' : 'var(--fw-card, #fff)', border: `1px solid ${filter === f.id ? '#0050a0' : 'var(--fw-cardBorder, #e0e0e0)'}`, borderRadius: 6, padding: '4px 6px', fontSize: 10, color: filter === f.id ? '#fff' : 'var(--fw-textSecondary, #888)', cursor: 'pointer', fontFamily: 'inherit', position: 'relative' }}>
                {f.label}
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filtered.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: 'var(--fw-textSecondary, #888)', fontSize: 12 }}>Sin conversaciones</div>}
          {filtered.map(room => (
            <div key={room.id} onClick={() => openRoom(room.id)}
              style={{ padding: '10px 12px', borderBottom: '1px solid var(--fw-cardBorder, #e0e0e0)', cursor: 'pointer', background: activeRoom === room.id ? 'var(--fw-card, #f0f4f8)' : 'transparent', display: 'flex', gap: 10, alignItems: 'center', transition: 'background .1s' }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: room.role === 'employee' ? 'rgba(34,197,94,.15)' : 'rgba(0,52,120,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
                {room.role === 'employee' ? '🔧' : '👤'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 4 }}>
                  ...
                  <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
                    {room.claimedBy && <span style={{ fontSize: 8, background: 'rgba(0,52,120,.08)', color: '#4a9eff', padding: '1px 5px', borderRadius: 4, fontWeight: 700 }}>{room.claimedBy}</span>}
      ...
      {activeRoom ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          {/* Header */}
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--fw-cardBorder, #e0e0e0)', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--fw-card, #fff)', flexShrink: 0 }}>
            ...
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: activeRoomInfo?.role === 'employee' ? 'rgba(34,197,94,.15)' : 'rgba(0,52,120,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
              {activeRoomInfo?.role === 'employee' ? '🔧' : '👤'}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--fw-text, #333)' }}>{activeRoomInfo?.name}</div>
              <div style={{ fontSize: 10, color: activeRoomInfo?.role === 'employee' ? '#22c55e' : '#4a9eff' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {!activeRoomInfo?.claimedBy && (
                <button onClick={(e) => { e.stopPropagation(); network.claimChat(activeRoom); }}
                  style={{ padding: '3px 8px', fontSize: 10, border: '1px solid rgba(0,52,120,.3)', borderRadius: 6, background: 'rgba(0,52,120,.08)', color: '#4a9eff', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700 }}>
                  Atender
                </button>
              )}
              {activeRoomInfo?.claimedBy && (
                <span style={{ fontSize: 9, color: '#4a9eff', padding: '3px 8px', background: 'rgba(0,52,120,.06)', borderRadius: 6 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {activeMessages.length === 0 && <div style={{ textAlign: 'center', color: 'var(--fw-textSecondary, #888)', fontSize: 12, marginTop: 30 }}>Sin mensajes todavía</div>}
            {activeMessages.map((msg, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                  ...
                  <div style={{ maxWidth: '80%', background: isMe ? '#003478' : 'var(--fw-card, #f0f0f0)', border: `1px solid ${isMe ? '#0050a0' : 'var(--fw-cardBorder, #e0e0e0)'}`, borderRadius: isMe ? '12px 4px 12px 12px' : '4px 12px 12px 12px', padding: '9px 13px', fontSize: 13, color: isMe ? '#fff' : 'var(--fw-text, #333)', lineHeight: 1.45 }}>{msg.text}</div>
                  <div style={{ fontSize: 9, color: 'var(--fw-textMuted, #999)', marginTop: 2 }}>{fmt(msg.ts)}</div>
                </div>
              );
            })}
            <div ref={bottomRef} />
          ...
          {/* Input */}
          <div style={{ padding: '10px 14px', borderTop: '1px solid var(--fw-cardBorder, #e0e0e0)', flexShrink: 0, background: 'var(--fw-bg, #fafafa)' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'var(--fw-card, #f5f5f5)', border: '1px solid var(--fw-cardBorder, #e0e0e0)', borderRadius: 12, padding: '4px 4px 4px 12px' }}>
              ...
              <button onClick={send} disabled={!inp.trim()}
                style={{ width: 36, height: 36, background: inp.trim() ? '#003478' : 'var(--fw-cardBorder, #ccc)', border: 'none', borderRadius: 9, cursor: inp.trim() ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background .15s' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={inp.trim() ? '#fff' : '#666'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13" /><path d="M22 2L15 22 11 13 2 9l20-7z" /></svg>
              </button>
            </div>
          </div>
        </div>
      ) : (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--fw-textMuted, #999)', fontSize: 14 }}>
